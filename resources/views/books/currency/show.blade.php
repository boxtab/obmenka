@extends('layouts.app', ['activePage' => 'currency', 'titlePage' => 'Код валюты: Добавление/Редактирование'])
                    ...
                    <form method="post"
                        ...
                        <div class="card ">
                            <div class="card-header card-header-success">
                                <h4 class="card-title">Код валюты</h4>
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('descr') ? ' has-danger' : '' }}">
                                            <input
                                                ...
                                                value="{{ $currency->descr ?? '' }}"
                                                required="true"
                                                aria-required="true"/>
                                            @if ($errors->has('descr'))
                                                <span id="name-error" class="error text-danger"
                                                      for="input-descr">{{ $errors->first('descr') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card-footer ml-auto mr-auto">
                                <div class="col-lg-6 text-left">
                                    <button type="submit" form="form-currency" class="btn btn-primary">Сохранить
                                    </button>
                                </div>
                                <div class="col-lg-6 text-right">
                                    <a href="{{ route('currency.index') }}" class="btn btn-block"
                                       role="button">Назад</a>
                                </div>
                            </div>
                        </div>
