...
class AddUserId extends Migration
{
    ...
    {
        Schema::table('offer', function (Blueprint $table) {
            $table->dropForeign('offer_created_user_id_foreign');
        });

        Schema::table('offer', function (Blueprint $table) {
            $table->dropForeign('offer_updated_user_id_foreign');
        });

        Schema::table('offer', function (Blueprint $table) {
            $table->dropForeign('offer_deleted_user_id_foreign');
        });

        if (Schema::hasColumn('offer', 'created_user_id')) {
            Schema::table('offer', function (Blueprint $table) {
                $table->dropColumn('created_user_id');
            });
        }

        if (Schema::hasColumn('offer', 'updated_user_id')) {
            Schema::table('offer', function (Blueprint $table) {
                $table->dropColumn('updated_user_id');
            });
        }

        if (Schema::hasColumn('offer', 'deleted_user_id')) {
            Schema::table('offer', function (Blueprint $table) {
