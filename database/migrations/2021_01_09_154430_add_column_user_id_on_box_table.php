...
class AddColumnUserIdOnBoxTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        Schema::table('box', function (Blueprint $table) {
            $table->dropForeign('box_created_user_id_foreign');
        });

        Schema::table('box', function (Blueprint $table) {
            $table->dropForeign('box_updated_user_id_foreign');
        });

        Schema::table('box', function (Blueprint $table) {
            $table->dropForeign('box_deleted_user_id_foreign');
        });

        if (Schema::hasColumn('box', 'created_user_id')) {
            Schema::table('box', function (Blueprint $table) {
                $table->dropColumn('created_user_id');
            });
        }

        if (Schema::hasColumn('box', 'updated_user_id')) {
            Schema::table('box', function (Blueprint $table) {
                $table->dropColumn('updated_user_id');
            });
        }

        if (Schema::hasColumn('box', 'deleted_user_id')) {
            Schema::table('box', function (Blueprint $table) {
